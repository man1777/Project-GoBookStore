	...
	public String save(Model model, @ModelAttribute("productForm") @Validated ProductModel entity,
		...
		if (result.hasErrors()) {
			...
			Product product = new Product(entity.getName(), entity.getPrice(), param.save(entity.getImage()),
					...
					manufacture, dateStr);
			System.out.println("Dữ liệu hình ảnh đưa về: "+ entity.getImage());
			System.out.println("Dữ liệu hình ảnh đưa về sau khi convert: "+ param.save(entity.getImage()));
			dao.save(product);
		}
...
System.out.println("keyword5 "+name);
		...
		System.out.println("productItem "+page);
