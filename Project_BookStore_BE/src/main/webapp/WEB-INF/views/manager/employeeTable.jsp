	...
	<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c" %>
		...
		<head>
			...
			<link
				...
				rel="stylesheet">
			<link rel="stylesheet" href="/manager/vendor/fontawesome-free/css/all.min.css">
											...
											<table class="table table-bordered" width="100%" cellspacing="0">
												...
												<tbody>
													<c:forEach var="item" items="${userItem.content}">
														<tr>
															<td>${item.group.fullname}</td>
															<td>${item.group.email}</td>
															<td>${item.position}</td>
															<td>${item.startDay}</td>
															<td>
																<%-- <a href="/admin/userForm/edit/${user.id}"
																	class="btn btn-success">Sửa</a> --%>
																	<button type="button" class="btn btn-danger"
																		data-toggle="modal"
																		data-target="#exampleModal${item.id}">Xóa</button>
																	<%-- <a
																		href="/admin/userForm/edit/employee/${item.id}"
																		class="btn btn-info">Nhân viên</a>
																		--%>
															</td>
														</tr>
														<!-- Modal -->
															...
															<div class="modal-dialog">
																<div class="modal-content">
																	...
																	<div class="modal-body">Tài khoản với email
																		${item.group.email} sẽ bị xóa chức vụ nhân viên.
																		Bạn có
																		muốn tiếp tục?</div>
																	<div class="modal-footer">
																		<button type="button" class="btn btn-secondary"
																			data-dismiss="modal">Đóng</button>
																		<a href="/admin/employeeTable/list/delete/${item.id}/${item.group.id}"
											...
											<nav aria-label="Page navigation example">
												<ul class="pagination justify-content-center">
													<li class="page-item ${(userItem.number==0)?'disabled':''}"><a
															class="page-link"
															href="/admin/employeeTable/list?p=${userItem.number-1}"
													...
													<li
														class="page-item ${(userItem.number>=userItem.totalPages-1)?'disabled':''}">
														<a class="page-link"
															href="/admin/employeeTable/list?p=${userItem.number+1}"
