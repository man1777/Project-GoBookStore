<template>
    <div>
        <v-data-table :headers="headers" :items="Orders_Back" class="elevation-1" search height="612px">
            <template v-slot:top>
                <v-toolbar flat>
                    <v-toolbar-title class="primary--text font-weight-medium text-h5">Đơn hàng hoàn
                        trả</v-toolbar-title>
                    <v-spacer></v-spacer>
                </v-toolbar>
            </template>
            ...
            <template v-slot:item.status="{ item }">
                <v-chip color="error" v-if="item.trangthai === 3">Hoàn trả đơn hàng</v-chip>
...
export default {
    data() {
        return {
            headers: [
                ...
                { text: "Lý do", value: "ghichu", sortable: false, align: "center" },
    ...
    methods: {
        API_Get_Order_B() {
            axios.get("http://localhost:8080/rest/ordersback").then(res => {
                this.Orders_Back = res.data
            }).catch(err => {
                console.log("err", err)
            })
        },
